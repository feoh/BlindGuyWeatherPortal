...
class OpenWeather_Graphics(displayio.Group):
    def __init__(self, root_group, *, am_pm=True, celsius=True):
        ...
        self.feels_like_text = Label(self.large_font)
        self.feels_like_text.x = 200
        self.feels_like_text.y = 195
        self.feels_like_text.color = 0xFFFFFF
        self._text_group.append(self.feels_like_text)
    ...
    def display_weather(self, weather):
        ...
        print(f"{feels_like=}")
        if self.celsius:
            self.temp_text.text = "A: %d °C" % temperature
            self.feels_like_text.text = "F: %d °C" % feels_like
        else:
            self.temp_text.text = "A: %d °F" % ((temperature * 9 / 5) + 32)
            self.feels_like_text.text = "F: %d °F" % ((feels_like * 9 / 5) + 32)
